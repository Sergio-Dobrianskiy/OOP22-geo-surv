...
ITickingObject è l'interfaccia alla base di tutte le entità presenti in gioco, prevede il metodo tick() che permette di definire il loro comportamento ad ogni loop. Viene implementata direttamente da Weapon e MonsterSpawner in quanto sono entità che hanno bisogno di influenzare il gioco senza avere una posizione una texture. 

L'interfaccia IGameObject prevede tutti i metodi necessari agli oggetti di gioco con una grafica e un sistema di collisioini. 

GameObject implementa ITickingObject e IGameObject ed è la classe astratta estesa da tutte le entità di gioco che non sono puramente logiche come le armi.
...
\end{figure}
Per gestire gli stati nel gioco è abbiamo pensato di implementare un sistema di stati che segue diagramma a stati finiti. Non siamo riusciti a implementare nella sua totalità questa idea per mancanza di tempo.
...
\begin{itemize}
	...
	\item \textbf{Loading}: stato durante il lo caricamento degli elementi di gioco.
	\item \textbf{Running}: stato durante il gioco vero e proprio, permette al gioco di aggiornare sia i metodi tick() che render() di tutti i GameObject.
	\item \textbf{Pause}: stato che mette il gioco in pausa, permette al gioco di aggiornare solamente il metodo render() di tutti i GameObject.
	\item \textbf{Won} e \textbf{Lost}: stato attivato dalla vittoria o dalla sconfitta del gioco.
\end{itemize}
...
\begin{itemize}
	...
	\item Gestione delle collisioni (java.awt)
	...
	\item Gestione dei controlli del giocatore
	...
	Come indicato nella sezione apposita, ho adattaro del codice preso dal progetto OOP21-ciccio-pier per la creazione della classe Texture e la gestione degli sprite.
\end{itemize}
\paragraph{}
...
La sfida è stata interessante perchè l'ambito è completamente al di fuori degli interessi personali e di quello a cui abituato lavorativamente parlando.
